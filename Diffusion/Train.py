...
def train(rank, world_size, modelConfig: Dict):
    ...
    dataloader = DataLoader(
        dataset,
        batch_size=modelConfig["batch_size"] // world_size,
        shuffle=False,
        num_workers=4,
        pin_memory=True,
        sampler=sampler,
    ...
    net_model = torch.nn.parallel.DistributedDataParallel(net_model, device_ids=[rank])
...
def eval(rank, world_size, modelConfig: Dict):
    torch.cuda.set_device(rank)
    device = torch.device(f"cuda:{rank}")
